...
function QtyCounter({ value, min = 1, max = 99, onChange }) {
  ...
  const commit = (n) => { const next = clamp(n); setVal(next); onChange?.(next) }
  ...
  return (
    <div className="qty-control" role="group" aria-label="수량 선택">
      <button type="button" className="qty-btn" onClick={dec} disabled={clamp(val) <= min} aria-label="수량 감소">−</button>
      <input
        className="qty-input"
        type="text"
        inputMode="numeric"
        ...
        aria-live="polite"
      />
      <button type="button" className="qty-btn" onClick={inc} disabled={clamp(val) >= max} aria-label="수량 증가">+</button>
    </div>
  )
}
/* ================================== */
...
const ItemCartForm = () => {
  ...
  useEffect(() => { if (userId) dispatch(fetchCartItemsThunk(userId)) }, [dispatch, userId])
          ...
          {!loading && cartItems.map((item, idx) => {
            ...
            return (
              <div className="cart-card" key={`${item.id ?? itemId}-${idx}`}>
                <button type="button" className="del-btn" onClick={() => handleDelete(itemId)} aria-label="삭제" title="삭제">x</button>
