...
function QnA() {
  ...
  const enterComment = useCallback(({ id, comment, onDone }) => {
    if (!comment?.trim()) {
  ...
  return (
    <section id="qna-list-section">
      ...
      {list.length === 0 && <p className="qna-empty">등록된 문의가 없습니다.</p>}
        ...
        <details className="qna-item" key={q.id}>
          ...
          <div className="qna-body">
            ...
            <div className="qna-actions">
              ...
              {!q?.comment && (
                <button
                  type="button"
                  className="btn"
                  onClick={() => setActiveQnaId((v) => (v === q.id ? null : q.id))}
                >
            ...
            {activeQnaId === q.id && !q?.comment && (
              <CommentBox
                id={q.id}
                onSubmit={(comment, done) => enterComment({ id: q.id, comment, onDone: done })}
              />
            )}
