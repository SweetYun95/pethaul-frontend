// src/pages/ItemLikePage.jsx
import { useEffect, useMemo, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { Link } from 'react-router-dom'
import {
  fetchMyLikedItemsThunk,
  fetchMyLikeIdsThunk,
  toggleLikeThunk,
} from '../features/likeSlice'
import './css/ItemLikePage.css' 
...
export default function ItemLikePage() {
  ...
  const filteredList = useMemo(() => {
    ...
    if (sellStatus) {
      arr = arr.filter(
        (item) => (item?.itemSellStatus ?? item?.sellStatus) === sellStatus
      )
    }
  ...
  return (
    <section className="ribbon-background" id="itemlike-list">
      <div className="ilp-container">{/* ✅ 인라인 스타일 제거 */}
        <div className="contents-card item">
          <div className="item-card-header">
            ...
            <div className="card-title-wrap">
              ...
              >
                {/* ✅ 아이콘 스타일을 CSS로 이동 */}
                <svg className="filter-icon" viewBox="0 0 24 24" role="img" aria-hidden="true">
                  <path d="M10 8.5h11m-18 0a2 2 0 1 0 4 0a2 2 0 0 0-4 0m0 7h11m3 0a2 2 0 1 0 4 0a2 2 0 0 0-4 0" />
                </svg>
              </button>
            </div>
          </div>
              ...
              <div className="filter-row">
                ...
                <div className="filter-bool">
                  <div className="segmented">
                    <button
                      type="button"
                      className={`seg-btn ${sellStatus === '' ? 'active' : ''}`}
                      onClick={() => setSellStatus('')}
                    >
                      전체
                    </button>
                    <button
                      type="button"
                      className={`seg-btn ${sellStatus === 'SELL' ? 'active' : ''}`}
                      onClick={() => setSellStatus('SELL')}
                    >
                      판매중
                    </button>
                    <button
                      type="button"
                      className={`seg-btn ${sellStatus === 'SOLD_OUT' ? 'active' : ''}`}
                      onClick={() => setSellStatus('SOLD_OUT')}
                    >
                      품절
                    </button>
                  </div>
              ...
              return (
                <Link key={item.id} to={`/items/detail/${item.id}`} className="card">
                  <div className="item-img like-btn">
                    ...
                    >
                      {/* 좋아요 아이콘(색상은 기존처럼 SVG 속성 그대로 유지) */}
                      {liked ? (
