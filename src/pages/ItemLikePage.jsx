...
export default function ItemLikePage() {
   ...
   // ====== 목록/카테고리 파생 ======
   const list = useMemo(() => (Array.isArray(likedItems) ? likedItems.filter(Boolean) : []), [likedItems])

   // 좋아요한 상품만 (이미 likedItems가 그것이지만 방어적으로 유지)
   const likedList = useMemo(() => list.filter((it) => !!likes[it.id]), [list, likes])
            ...
            <div className="contents-card item">
               <div className="item-card-header">
                  ...
                  <div className="card-title-wrap">
                     ...
                     {/* 필터 버튼 */}
                     <button type="button" className="filter-toggle-btn" onClick={() => setIsFilterOpen((v) => !v)} aria-expanded={isFilterOpen} aria-controls="like-filter-panel" title="필터 열기/닫기">
                              ...
                              <button className={`like ${liked ? 'on' : ''}`} aria-label={liked ? '좋아요 취소' : '좋아요'} onClick={(e) => handleLike(e, item.id)} type="button" title={liked ? '좋아요 취소' : '좋아요'}>
                                 {liked ? (
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24">
                                       <path
                                          fill="#000"
                                          d="M22 6V5h-1V4h-1V3h-6v1h-1v1h-2V4h-1V3H4v1H3v1H2v1H1v5h1v1h1v1h1v1h1v1h1v1h1v1h1v1h1v1h1v1h2v-1h1v-1h1v-1h1v-1h1v-1h1v-1h1v-1h1v-1h1V6zm-2 4v1h-1v1h-1v1h-1v1h-1v1h-1v1h-1v1h-1v1h-2v-1h-1v-1H9v-1H8v-1H7v-1H6v-1H5v-1H4v-1H3V7h1V6h1V5h4v1h1v1h1v1h2V7h1V6h1V5h4v1h1v1h1v3z"
                                       ></path>
                                    </svg>
                                 )}
                              </button>
